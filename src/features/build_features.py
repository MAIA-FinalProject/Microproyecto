...
transforma en una matriz numérica lista para modelado: encoding de
variables categóricas, imputación de nulos y normalización de variables
continuas.
...
def _impute_continuous(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """Imputa continuas remanentes con la mediana (robusta a outliers,
    p.ej. el caso de Apgar=1 documentado en el EDA)."""
    df = df.copy()
    for col in cols:
...
def _normalize_continuous(df: pd.DataFrame, cols: list[str]) -> pd.DataFrame:
    """Normalización z-score de variables continuas."""
    df = df.copy()
    for col in cols:
        if col in df.columns:
...
def build_features(df: pd.DataFrame | None = None) -> pd.DataFrame:
    ...
    Returns
    -------
    DataFrame numérico listo para modelado, con la columna TARGET
    codificada como 0/1.
    """
    if df is None:
        df = load_labeled()
    ...
    continuous_cols = [c for c in CONTINUOUS_VARS if c in df.columns]
    df = _impute_continuous(df, continuous_cols)
    df = _normalize_continuous(df, continuous_cols)
    ...
    # Imputar binarias restantes con la moda (0 = "no" es la categoría
    # dominante en casi todas las variables clínicas de este dataset).
